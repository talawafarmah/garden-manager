...
export default function WishlistCatalog() {
  ...
  useEffect(() => {
    const fetchCatalogData = async () => {
      ...
      try {
        ...
        // FIX: Safely check expiry date so blank inputs don't crash or trigger the block
  ...
  useEffect(() => {
    ...
    const loadSignedUrls = async () => {
      ...
      const urlsToFetch = seeds
        ...
        .filter(img => img && typeof img === 'string' && img.trim() !== '' && !img.startsWith('data:') && !img.startsWith('http')) as string[]; 
  ...
  // FIX: Added "Make Changes" button to allow editing until expiry
  if (isSuccess) {
    return (
      <div className="min-h-screen bg-stone-50 flex items-center justify-center p-4 text-center">
        <div className="bg-white p-8 rounded-3xl shadow-xl max-w-md w-full border border-stone-200 animate-in zoom-in-95 duration-500">
          ...
          <p className="text-stone-500 text-sm mb-6">Thank you, {session.list_name}! Your garden requests for {seasonName} have been recorded. You can return to this link to update your choices anytime before it expires.</p>
          <button 
            onClick={() => setIsSuccess(false)} 
            className="px-6 py-3 bg-stone-100 text-stone-700 font-bold rounded-xl hover:bg-stone-200 active:scale-95 transition-all shadow-sm"
          >
            Make Changes
          </button>
        </div>
