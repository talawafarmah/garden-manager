...
export default function AdminSeasons({ handleGoBack, userRole }: Props) {
  ...
  const handleCreateSeason = async (e: React.FormEvent) => {
    ...
    if (!error && data) { setSeasons([data as Season, ...seasons]); setActiveSeasonId(data.id); setNewSeasonName(""); }
  };
  ...
  const startEditingSeason = () => {
    ...
    if (s) { setEditSeasonName(s.name); setEditSeasonStatus(s.status); setEditingSeason(true); }
  };
  ...
  const saveSeasonEdit = async () => {
    ...
    if (!error) { setSeasons(seasons.map(s => s.id === activeSeasonId ? { ...s, name: editSeasonName.trim(), status: editSeasonStatus } : s)); setEditingSeason(false); }
  };
        ...
        <section className="bg-white p-6 rounded-3xl shadow-sm border border-stone-200">
          ...
          <div className="border-t border-stone-100 pt-4">
            ...
            <form onSubmit={handleCreateSeason} className="flex gap-2">
              <input type="text" placeholder="e.g. Spring 2026" value={newSeasonName} onChange={e => setNewSeasonName(e.target.value)} className="flex-1 bg-stone-50 border border-stone-200 rounded-xl p-3 text-sm outline-none focus:border-emerald-500" />
              <button type="submit" disabled={!newSeasonName.trim()} className="bg-stone-800 text-white px-5 rounded-xl font-bold text-xs uppercase tracking-wider hover:bg-stone-700 active:scale-95 transition-all disabled:opacity-50">Add</button>
            </form>
          </div>
        </section>
                        ...
                        <div className="flex justify-between items-start">
                          <div>
                            <h4 className="font-bold text-stone-800 flex items-center gap-2">
                              {session.list_name}
                              {isSubmitted && <span className="bg-blue-100 text-blue-700 text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded">Submitted</span>}
                            </h4>
