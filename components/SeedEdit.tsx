...
export default function SeedEdit({ seed, inventory, setInventory, categories, setCategories, navigateTo, handleGoBack }: any) {
  ...
  const handleEditPhotoCapture = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => { setEditFormData({ ...editFormData, images: [...(editFormData.images || []), reader.result as string] }); };
  ...
  const handleAutoFill = async () => {
    ...
    try {
      ...
      if (textResponse) {
        ...
        for (const key in parsedData) { if (parsedData[key] === null) parsedData[key] = ""; }
    ...
    <main className="min-h-screen bg-stone-50 text-stone-900 pb-20 font-sans">
      ...
      <header className="bg-white p-4 shadow-sm sticky top-0 z-10 flex items-center justify-between border-b border-stone-200">
        <div className="flex items-center gap-3">
          <button onClick={handleCancel} className="p-2 text-stone-500 hover:bg-stone-100 rounded-full transition-colors"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
          <h1 className="text-xl font-bold text-stone-800">{inventory.some((s: InventorySeed) => s.id === seed.id) ? 'Edit Seed' : 'New Seed'}</h1>
        </div>
        <button onClick={handleSaveEdit} disabled={isSaving} className="px-5 py-2 bg-emerald-600 text-white font-black rounded-xl shadow-sm disabled:opacity-50 transition-colors hover:bg-emerald-700 tracking-wide">{isSaving ? 'Saving...' : 'Save'}</button>
      </header>

      <div className="max-w-md mx-auto p-4 space-y-5">
        <div className="grid grid-cols-2 gap-3">
           <button onClick={() => setIsImageSearchOpen(true)} className="py-4 bg-blue-600 text-white font-black rounded-xl shadow-md text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 active:scale-95 transition-all">🌐 Internet Search</button>
           <button onClick={handleAutoFill} disabled={isAutoFilling} className="py-4 bg-indigo-600 text-white font-black rounded-xl shadow-md text-[10px] uppercase tracking-widest flex items-center justify-center gap-2 active:scale-95 transition-all disabled:opacity-50">✨ Magic AutoFill</button>
        </div>
        ...
        <section className="bg-white p-5 rounded-3xl shadow-sm border border-stone-200">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => editPhotoInputRef.current?.click()} className="text-emerald-600 text-xs font-black uppercase tracking-widest flex items-center gap-1 bg-emerald-50 px-3 py-2 rounded-lg border border-emerald-100 shadow-sm active:scale-95 transition-transform">Add Photo</button>
        ...
        <section className="bg-white p-6 rounded-3xl shadow-sm border border-stone-200 space-y-4">
          ...
          <div><label className="block text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1.5 ml-1">Shortcode ID</label><input type="text" value={editFormData.id} onChange={(e) => setEditFormData({ ...editFormData, id: e.target.value.toUpperCase() })} className="w-full bg-stone-50 border border-stone-200 rounded-xl p-3 font-mono uppercase font-bold outline-none focus:border-emerald-500 transition-colors shadow-sm" /></div>
        ...
        <section className="bg-white p-6 rounded-3xl shadow-sm border border-stone-200 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1.5 ml-1">Days to Maturity</label><input type="number" value={editFormData.days_to_maturity || ''} onChange={(e) => setEditFormData({ ...editFormData, days_to_maturity: e.target.value })} className="w-full bg-stone-50 border border-stone-200 rounded-xl p-3 text-sm font-bold shadow-sm outline-none focus:border-emerald-500" /></div>
            <div><label className="block text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1.5 ml-1">Lifecycle</label><input type="text" value={editFormData.lifecycle || ''} onChange={(e) => setEditFormData({ ...editFormData, lifecycle: e.target.value })} className="w-full bg-stone-50 border border-stone-200 rounded-xl p-3 text-sm shadow-sm outline-none focus:border-emerald-500" /></div>
            <div><label className="block text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1.5 ml-1">Sunlight</label><input type="text" value={editFormData.sunlight || ''} onChange={(e) => setEditFormData({ ...editFormData, sunlight: e.target.value })} className="w-full bg-stone-50 border border-stone-200 rounded-xl p-3 text-sm shadow-sm outline-none focus:border-emerald-500" /></div>
            <div><label className="block text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1.5 ml-1">Germination Days</label><input type="text" value={editFormData.germination_days || ''} onChange={(e) => setEditFormData({ ...editFormData, germination_days: e.target.value })} className="w-full bg-stone-50 border border-stone-200 rounded-xl p-3 text-sm shadow-sm outline-none focus:border-emerald-500" /></div>
          </div>

          <div className="grid grid-cols-3 gap-2">
            <div><label className="block text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1.5 ml-1">Seed Depth</label><input type="text" value={editFormData.seed_depth || ''} onChange={(e) => setEditFormData({ ...editFormData, seed_depth: e.target.value })} className="w-full bg-stone-50 border border-stone-200 rounded-xl p-2 text-xs shadow-sm outline-none focus:border-emerald-500" /></div>
            <div><label className="block text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1.5 ml-1">Plant Space</label><input type="text" value={editFormData.plant_spacing || ''} onChange={(e) => setEditFormData({ ...editFormData, plant_spacing: e.target.value })} className="w-full bg-stone-50 border border-stone-200 rounded-xl p-2 text-xs shadow-sm outline-none focus:border-emerald-500" /></div>
            <div><label className="block text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1.5 ml-1">Row Space</label><input type="text" value={editFormData.row_spacing || ''} onChange={(e) => setEditFormData({ ...editFormData, row_spacing: e.target.value })} className="w-full bg-stone-50 border border-stone-200 rounded-xl p-2 text-xs shadow-sm outline-none focus:border-emerald-500" /></div>
          </div>
        ...
        <section className="bg-white p-6 rounded-3xl shadow-sm border border-stone-200 space-y-4">
          ...
          <div>
            ...
            <input type="text" value={(editFormData.companion_plants || []).join(', ')} onChange={(e) => setEditFormData({ ...editFormData, companion_plants: e.target.value.split(',').map((s: string) => s.trim()).filter(Boolean) })} className="w-full bg-stone-50 border border-stone-200 rounded-xl p-3 text-sm font-medium shadow-sm outline-none focus:border-emerald-500" />
          </div>
          <div>
            <label className="block text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1.5 ml-1">Growing Notes</label>
            <textarea value={editFormData.notes || ''} onChange={(e) => setEditFormData({ ...editFormData, notes: e.target.value })} rows={5} className="w-full bg-stone-50 border border-stone-200 rounded-xl p-3 text-sm shadow-sm resize-none outline-none focus:border-emerald-500" placeholder="Vendor tips, germination notes, past performance..." />
          </div>
        </section>

        {inventory.some((s: InventorySeed) => s.id === seed.id) && (
          <button onClick={handleDeleteSeed} className="w-full py-4 mt-2 bg-red-50 text-red-600 font-bold rounded-2xl border border-red-200 hover:bg-red-100 transition-colors flex items-center justify-center gap-2 shadow-sm">
