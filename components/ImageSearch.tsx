...
const ImageSearch: React.FC<ImageSearchProps> = ({ query: initialQuery = '', onSelect, onClose }) => {
  ...
  const performSearch = async (e?: React.FormEvent) => {
    ...
    // Using Wikipedia's open Action API to bypass all Gemini authentication and 403 errors.
    // We search for the query, ask for pageimages, and request high-res thumbnails.
    ...
    try {
      ...
      if (pages) {
        ...
        // Convert the pages object into an array and filter only those that actually have images
        ...
        if (fetchedResults.length > 0) {
          ...
          throw new Error("No botanical images found for this specific query.");
        }
      } else {
        throw new Error("No articles found matching this plant variety.");
      }
    } catch (err: any) {
      ...
      <div className="w-full max-w-2xl overflow-hidden rounded-3xl bg-slate-900 border border-slate-800 shadow-2xl flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between border-b border-slate-800 p-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-white">Open Botanical Search</h3>
              <p className="text-xs text-slate-400">Retrieving images via Wikimedia Commons</p>
            </div>
          </div>
        ...
        <div className="p-5 bg-slate-900/50">
          <form onSubmit={performSearch} className="relative">
            <input
              ...
              placeholder="e.g. 'Jalapeno plant', 'Solanum lycopersicum'"
        ...
        <div className="flex-1 overflow-y-auto p-5 custom-scrollbar min-h-[300px]">
          {isSearching && (
            <div className="flex flex-col items-center justify-center py-20 gap-4 text-slate-400">
              ...
              <p className="animate-pulse text-sm font-medium text-emerald-500/80">Accessing Wikimedia Commons...</p>
            </div>
          )}
          ...
          {!isSearching && results.length === 0 && !error && (
            <div className="flex flex-col items-center justify-center py-20 text-slate-500 text-center">
              ...
              <p className="text-sm max-w-xs">Enter a plant variety to search the world's largest open botanical database.</p>
              <p className="text-xs mt-2 text-slate-600">Pro tip: Include the word "plant" or use scientific names for better results.</p>
            </div>
          )}
                ...
                >
                  <div className="aspect-video w-full bg-slate-950 flex items-center justify-center overflow-hidden">
                    <img
                      ...
                      onError={(e) => {
                        (e.target as HTMLImageElement).src = `https://via.placeholder.com/400x300?text=${encodeURIComponent(img.title)}`;
                      }}
                    />
                  </div>
                  <div className="p-3">
