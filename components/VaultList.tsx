...
export default function VaultList({ inventory, setInventory, categories, isLoadingDB, navigateTo, handleGoBack }: any) {
  ...
  const toggleOutOfStock = async (seed: InventorySeed, e: React.MouseEvent) => {
    ...
    setInventory(inventory.map((s: InventorySeed) => s.id === seed.id ? { ...s, out_of_stock: newStatus } : s));
  ...
  const filteredInventory = inventory.filter((seed: InventorySeed) => {
    ...
    <main className="min-h-screen bg-stone-50 text-stone-900 pb-20">
          ...
          <div className="bg-white rounded-2xl p-5 w-full max-w-sm max-h-[80vh] flex flex-col shadow-2xl animate-in zoom-in-95 duration-200">
             ...
             <div className="overflow-y-auto flex-1 space-y-3 pr-1 scrollbar-hide">
               ...
               {(companionModalSeed.companion_plants || []).map(comp => {
                 const matches = inventory.filter((s: InventorySeed) =>
                   !s.out_of_stock && s.id !== companionModalSeed.id &&
      ...
      <div className="max-w-md mx-auto p-4 space-y-4">
        <div className="relative">
          <input type="text" placeholder="Search by name, category, or code..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-white border border-stone-200 rounded-xl py-3 pl-10 pr-4 shadow-sm focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 outline-none transition-all" />
        ...
        <div className="space-y-3">
          {isLoadingDB ? (
            ...
            filteredInventory.map((seed: InventorySeed) => {
              ...
              return (
                <div key={seed.id} onClick={() => navigateTo('seed_detail', seed)} className={`bg-white p-3 rounded-xl border ${isOutOfStock ? 'border-red-100 bg-stone-50/50 opacity-75' : 'border-stone-100'} shadow-sm flex flex-col gap-3 hover:border-emerald-400 hover:shadow-md transition-all active:scale-95 cursor-pointer`}>
                  ...
                  <div className="flex justify-between items-center border-t border-stone-100 pt-2">
                    <button onClick={(e) => { e.stopPropagation(); setCompanionModalSeed(seed); setCompanionInStockOnly(false); }} className="text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-md transition-colors border bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100 flex items-center gap-1.5"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>Companions</button>
