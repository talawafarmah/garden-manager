...
IMPORTANT: You MUST respond ONLY with a valid JSON object. Do not include markdown formatting wrappers like \`\`\`json. The JSON must exactly match this structure, substituting null for entirely unknown numeric values and empty strings for unknown text:
{
  ...
  const analyzeUrl = async () => {
    if (!importUrl.trim() || !importUrl.startsWith("http")) { 
      setErrorMsg("Please enter a valid complete URL."); 
      return; 
    }
  ...
  const handleSaveScannedToInventory = async () => {
    if (analysisResult) {
      ...
      const newSeedPayload: any = {
        ...
        days_to_maturity: analysisResult.days_to_maturity !== undefined && analysisResult.days_to_maturity !== null ? Number(analysisResult.days_to_maturity) : '', 
        ...
        stratification_days: analysisResult.stratification_days || '', 
        ...
        thumbnail: '', 
        scoville_rating: analysisResult.scoville_rating !== undefined && analysisResult.scoville_rating !== null ? Number(analysisResult.scoville_rating) : ''
      };

      if (showNewCatForm && newCatName.trim() !== '') {
          newSeedPayload.newCatName = newCatName;
        ...
        {analysisResult ? (
          <div className="w-full max-w-sm animate-in slide-in-from-bottom-4 duration-500 pb-10 space-y-4">
            ...
            <div className="bg-stone-800 rounded-2xl p-5 shadow-xl border border-stone-700 space-y-4">
                ...
                <div className="p-4 bg-stone-900/50 border border-emerald-900 rounded-xl space-y-3 relative overflow-hidden">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-stone-900 border border-stone-700 rounded-md p-2 text-sm text-stone-100 outline-none" 
                    />
                  </div>
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full bg-stone-900 border border-stone-700 rounded-md p-2 text-sm text-stone-100 font-mono uppercase outline-none" 
                    />
                  </div>
                </div>
              )}
              ...
              {isPepper && (
                 <div className="bg-red-900/20 p-3 rounded-lg border border-red-900/50 flex items-center justify-between gap-4">
                    ...
                    <input 
                      ...
                      onChange={(e) => setAnalysisResult({ ...analysisResult, scoville_rating: e.target.value as any })} 
                      placeholder="e.g. 50000" 
                      className="w-1/2 bg-stone-900 border border-red-900/50 rounded-md p-2 text-stone-100 font-bold outline-none focus:border-red-500" 
                    />
                 </div>
              )}
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-stone-900 border border-stone-700 rounded-lg p-3 text-stone-100 font-bold focus:border-emerald-500 outline-none" 
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-stone-900 border border-stone-700 rounded-lg p-2 text-stone-100 italic focus:border-emerald-500 outline-none" 
                  />
                </div>
                <div>
                  ...
                  <input 
                    ...
                    className="w-full bg-stone-900 border border-stone-700 rounded-lg p-2 text-stone-100 focus:border-emerald-500 outline-none" 
                  />
                </div>
                <div>
                  ...
                  <input 
                    type="text" 
                    value={analysisResult.lifecycle || ''} 
                    onChange={(e) => setAnalysisResult({ ...analysisResult, lifecycle: e.target.value })} 
                ...
                <div>
                  ...
                  <input 
                    type="text" 
                    value={analysisResult.sunlight || ''} 
                    onChange={(e) => setAnalysisResult({ ...analysisResult, sunlight: e.target.value })} 
            ...
            <div className="bg-stone-800 rounded-2xl p-5 shadow-xl border border-stone-700 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input 
                    type="text" 
                    value={analysisResult.seed_depth || ''} 
                    onChange={(e) => setAnalysisResult({ ...analysisResult, seed_depth: e.target.value })} 
                    className="w-full bg-stone-900 border border-stone-700 rounded-lg p-2 text-stone-100 focus:border-emerald-500 outline-none" 
                  />
                </div>
                <div>
                  ...
                  <input 
                    type="text" 
                    value={analysisResult.plant_spacing || ''} 
                    onChange={(e) => setAnalysisResult({ ...analysisResult, plant_spacing: e.target.value })} 
                    className="w-full bg-stone-900 border border-stone-700 rounded-lg p-2 text-stone-100 focus:border-emerald-500 outline-none" 
                  />
                </div>
                <div>
                  ...
                  <input 
                    type="text" 
                    value={analysisResult.row_spacing || ''} 
                    onChange={(e) => setAnalysisResult({ ...analysisResult, row_spacing: e.target.value })} 
                    className="w-full bg-stone-900 border border-stone-700 rounded-lg p-2 text-stone-100 focus:border-emerald-500 outline-none" 
                  />
                </div>
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => setAnalysisResult({ ...analysisResult, days_to_maturity: e.target.value as any })} 
                    className="w-full bg-stone-900 border border-stone-700 rounded-lg p-2 text-stone-100 focus:border-emerald-500 outline-none" 
                  />
                </div>
              </div>
            ...
            <div className="bg-stone-800 rounded-2xl p-5 shadow-xl border border-stone-700 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="col-span-2">
                  ...
                  <input 
                    type="text" 
                    value={analysisResult.germination_days || ''} 
                    onChange={(e) => setAnalysisResult({ ...analysisResult, germination_days: e.target.value })} 
                ...
                <div className="flex items-center gap-2 bg-stone-900 p-2 rounded-lg border border-stone-700">
                  <input 
                    type="checkbox" 
                    checked={analysisResult.light_required || false} 
                    onChange={(e) => setAnalysisResult({ ...analysisResult, light_required: e.target.checked })} 
                ...
                <div className="flex items-center gap-2 bg-stone-900 p-2 rounded-lg border border-stone-700">
                  <input 
                    type="checkbox" 
                    checked={analysisResult.cold_stratification || false} 
                    onChange={(e) => setAnalysisResult({ ...analysisResult, cold_stratification: e.target.checked })} 
                    className="w-4 h-4 accent-emerald-500" 
                  />
                ...
                {analysisResult.cold_stratification && (
                  <div className="col-span-2 mt-2">
                    ...
                    <input 
                      ...
                      onChange={(e) => setAnalysisResult({ ...analysisResult, stratification_days: e.target.value as any })} 
            ...
            <div className="bg-stone-800 rounded-2xl p-5 shadow-xl border border-stone-700 space-y-4">
              <div>
                ...
                <input 
                  type="text" 
                  value={(analysisResult.companion_plants || []).join(', ')} 
                  onChange={(e) => setAnalysisResult({ ...analysisResult, companion_plants: e.target.value.split(',').map(s=>s.trim()).filter(Boolean) })} 
                  placeholder="Basil, Marigold..." 
                  className="w-full bg-stone-900 border border-stone-700 rounded-lg p-3 text-stone-100 focus:border-emerald-500 outline-none" 
                />
              </div>
              <div>
                ...
                <textarea 
                  ...
                  rows={4} 
                  className="w-full bg-stone-900 border border-stone-700 rounded-lg p-3 text-stone-100 focus:border-emerald-500 outline-none resize-none" 
                />
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-4">
              <button 
                ...
                className="flex-1 py-4 bg-stone-700 rounded-xl font-medium hover:bg-stone-600 transition-colors"
              >
                Back
              </button>
              <button 
                onClick={handleSaveScannedToInventory} 
                className="flex-[2] py-4 bg-emerald-600 rounded-xl font-bold hover:bg-emerald-500 shadow-lg shadow-emerald-900/50 transition-all"
              >
                Review & Save
              </button>
            </div>
          ...
          <div className="w-full flex flex-col items-center animate-in fade-in duration-300">
            {imagePreview ? (
              <>
                ...
                <div className="flex gap-4 w-full max-w-sm">
                  <button 
                    ...
                    disabled={isAnalyzing} 
                    className="flex-1 py-4 bg-stone-800 rounded-xl font-medium hover:bg-stone-700 border border-stone-700 disabled:opacity-50"
                  >
                    Retake
                  </button>
                  <button 
                    ...
                    disabled={isAnalyzing} 
                    className="flex-1 py-4 bg-emerald-600 rounded-xl font-bold hover:bg-emerald-500 shadow-lg shadow-emerald-900/50 flex items-center justify-center gap-2 disabled:opacity-50"
                  >
              ...
              <button 
                onClick={() => fileInputRef.current?.click()} 
                className="flex flex-col items-center justify-center w-full max-w-sm aspect-[3/4] border-2 border-dashed border-stone-600 rounded-3xl bg-stone-800/50 text-stone-400 hover:text-emerald-400 hover:border-emerald-50 active:scale-95 transition-all"
              >
