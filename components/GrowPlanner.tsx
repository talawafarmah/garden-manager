...
// Intelligent Nursery Time Guesser
const estimateNurseryWeeks = (category: string) => {
  const cat = (category || '').toLowerCase();
  if (cat.includes('pepper') || cat.includes('eggplant')) return 8;
  if (cat.includes('tomato')) return 6;
  if (cat.includes('brassica') || cat.includes('cabbage') || cat.includes('broccoli') || cat.includes('kale')) return 4;
  if (cat.includes('herb') || cat.includes('flower')) return 6;
  if (cat.includes('squash') || cat.includes('melon') || cat.includes('cucumber') || cat.includes('pumpkin')) return 3;
  if (cat.includes('root') || cat.includes('radish') || cat.includes('carrot') || cat.includes('bean') || cat.includes('pea') || cat.includes('corn')) return 0; // Direct sow
...
export default function GrowPlanner({ navigateTo, handleGoBack, userRole }: Props) {
  ...
  // FIX: Updated the state to explicitly use the DemandQueueItem interface
  ...
  useEffect(() => {
    if (activeSeasonId) fetchPlannerData();
  }, [activeSeasonId]);

  const fetchPlannerData = async () => {
    setIsLoading(true);
    try {
      // 1. Get Existing Grow Plans
      ...
      // 2. Get Demand to find Unplanned items
        ...
        (selections || []).forEach(sel => {
          if (sel.seed_id && sel.seed && !plannedSeedIds.has(sel.seed_id)) {
            if (demandMap.has(sel.seed_id)) {
              demandMap.get(sel.seed_id)!.requested_qty += 1;
            } else {
              demandMap.set(sel.seed_id, { seed: sel.seed, requested_qty: 1 });
            }
          }
        });
  ...
  const openPlanModal = (item: DemandQueueItem) => {
    setEditingItem(item);
    setFormWeeks(estimateNurseryWeeks(item.seed.category));
    setFormTargetDate(globalTargetDate);
    setFormQty(Math.max(item.requested_qty, 1)); // Default to requested
    ...
    <main className="min-h-screen bg-stone-50 text-stone-900 pb-24 font-sans relative">
      <header className="bg-stone-900 text-white p-4 shadow-md sticky top-0 z-10 flex items-center justify-between">
        ...
        <select 
          ...
          className="bg-stone-800 border border-stone-700 text-sm font-bold rounded-xl px-3 py-1.5 outline-none appearance-none cursor-pointer"
        >
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-6">
        ...
        <div className="bg-amber-50 p-4 rounded-3xl border border-amber-200 shadow-sm flex items-center justify-between">
          <div>
            <h2 className="font-black text-amber-900 text-sm">Target Frost Date</h2>
            <p className="text-[10px] text-amber-700 uppercase tracking-widest mt-0.5">Used for calculations</p>
          </div>
          <input 
            type="date" 
            value={globalTargetDate} 
            onChange={(e) => setGlobalTargetDate(e.target.value)} 
            className="bg-white border border-amber-300 rounded-xl px-3 py-2 text-sm font-bold text-amber-900 outline-none focus:border-amber-500 shadow-sm"
          />
        </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col md:flex-row gap-6">
                ...
                <div className="flex flex-col gap-2 max-h-[400px] overflow-y-auto pr-1">
                  {demandQueue.map(item => (
                    <div key={item.seed.id} className="bg-white p-3 rounded-2xl border border-stone-200 shadow-sm group hover:border-emerald-300 transition-colors">
                      ...
                      <button onClick={() => openPlanModal(item)} className="w-full bg-stone-100 text-stone-600 hover:bg-emerald-50 hover:text-emerald-700 text-xs font-bold py-1.5 rounded-lg transition-colors border border-transparent hover:border-emerald-200 flex items-center justify-center gap-1">
            ...
            <div className="md:w-2/3 space-y-4">
              <h3 className="font-black text-xs uppercase tracking-widest text-stone-400 border-b border-stone-200 pb-2 flex justify-between">
                <span>Seed Starting Calendar</span>
                <span className="text-emerald-600">{sortedPlans.length} Scheduled</span>
              </h3>

              {sortedPlans.length === 0 ? (
                <div className="bg-white p-10 rounded-3xl border border-stone-200 text-center shadow-sm">
                  <div className="w-16 h-16 bg-stone-100 rounded-full flex items-center justify-center mx-auto mb-3 text-2xl">🌱</div>
                  <h4 className="font-black text-stone-800">Your calendar is empty</h4>
                  <p className="text-xs text-stone-500 mt-1">Schedule seeds from the queue to build your timeline.</p>
                </div>
              ) : (
                      ...
                      <div key={plan.id} className="relative pl-6">
                        ...
                        <div className="bg-white p-4 rounded-2xl border border-stone-200 shadow-sm flex flex-col gap-2 hover:shadow-md transition-shadow">
                          <div className="flex justify-between items-start">
                            <div>
                              <span className={`text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded border inline-block mb-1.5 ${badgeColor}`}>{statusTxt}</span>
                              <h4 className="font-black text-stone-800 text-lg leading-tight cursor-pointer hover:text-emerald-600" onClick={() => navigateTo('seed_detail', plan.seed)}>{plan.seed?.variety_name}</h4>
                            </div>
                            <div className="text-right">
                              <div className="text-xl font-black text-emerald-600">{planDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</div>
                              <div className="text-[10px] text-stone-400 font-bold uppercase tracking-widest mt-0.5">Start Date</div>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between mt-2 pt-3 border-t border-stone-100">
                             <div className="flex items-center gap-3 text-xs font-bold text-stone-500">
                               <span className="flex items-center gap-1"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg> Qty: {plan.planned_qty}</span>
                               <span>{plan.indoor_start_date === plan.target_plant_date ? 'Direct Sow' : 'Start Indoors'}</span>
                             </div>
                             
                             <div className="flex gap-1.5">
                               <button onClick={() => deletePlan(plan.id, plan.seed!, plan.planned_qty)} className="p-1.5 text-stone-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Remove from plan"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                               <button onClick={() => navigateTo('tray_edit')} className="px-3 py-1.5 bg-emerald-50 text-emerald-700 font-black text-[10px] uppercase tracking-widest rounded-lg border border-emerald-200 hover:bg-emerald-100 transition-colors flex items-center gap-1"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M12 4v16m8-8H4" /></svg> Sow Trays</button>
                             </div>
                          </div>
                        </div>
                      </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="bg-stone-50 p-4 border-b border-stone-200 flex justify-between items-center">
              <div>
                ...
                <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mt-0.5">{editingItem.seed.variety_name}</p>
              </div>
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                 <div className="bg-stone-50 p-3 rounded-xl border border-stone-200 text-center">
                    ...
                    <div className="flex items-center justify-center gap-1 text-sm font-black text-stone-800">
                      <input type="number" min="0" value={formWeeks} onChange={(e) => setFormWeeks(Number(e.target.value))} className="w-10 text-center border-b border-stone-300 outline-none bg-transparent focus:border-emerald-500" /> Weeks
                    </div>
                 </div>
                 <div className="bg-stone-50 p-3 rounded-xl border border-stone-200 text-center">
              ...
              <div className="bg-emerald-50 border border-emerald-200 p-4 rounded-2xl flex justify-between items-center mt-2">
                ...
                <span className="text-xl font-black text-emerald-600">{new Date(calculateStartDate(formTargetDate, formWeeks, editingItem.seed.germination_days)).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
              </div>

              <button onClick={savePlan} className="w-full py-4 bg-emerald-600 text-white rounded-xl font-black uppercase tracking-widest shadow-xl shadow-emerald-900/20 active:scale-95 transition-all mt-2 hover:bg-emerald-500">
