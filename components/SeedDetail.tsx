...
import { InventorySeed, SeedlingTray, AppView } from '../types';
...
export default function SeedDetail({ seed, trays, navigateTo, handleGoBack }: any) {
  ...
  useEffect(() => { setViewingImageIndex(seed.primaryImageIndex || 0); }, [seed]);

  const displayImg = (seed.images && seed.images.length > 0) ? seed.images[viewingImageIndex] : null;
  const seedHistory = trays.filter((t: SeedlingTray) => t.contents.some(c => c.seed_id === seed.id));
  ...
  return (
    <main className="min-h-screen bg-stone-50 text-stone-900 pb-20">
      ...
      <header className="bg-emerald-700 text-white p-4 shadow-md sticky top-0 z-10 flex items-center justify-between">
        ...
        <button onClick={() => navigateTo('seed_edit', seed)} className="p-2 bg-emerald-800 rounded-full hover:bg-emerald-600 transition-colors flex items-center gap-1 px-3">
      ...
      <div className="max-w-md mx-auto">
        <div className="w-full aspect-[4/3] bg-stone-200 relative">
          {displayImg ? (
            <img src={displayImg} alt={seed.variety_name} className="w-full h-full object-cover cursor-zoom-in" onClick={() => setFullScreenImage(displayImg)} />
          ) : (
          ...
          <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-stone-900/80 to-transparent p-4 pt-12 pointer-events-none">
            <div className="flex gap-2 mb-1">
              <div className="bg-emerald-500 text-white text-xs font-bold px-2 py-1 rounded shadow-sm">{seed.id}</div>
              {seed.out_of_stock && <div className="bg-red-500 text-white text-xs font-bold px-2 py-1 rounded shadow-sm">OUT OF STOCK</div>}
            </div>
            <h2 className="text-2xl font-bold text-white leading-tight">{seed.variety_name}</h2>
            <p className="text-emerald-300 text-sm font-medium">{seed.category} <span className="text-stone-300 font-normal italic">({seed.species})</span></p>
          </div>
        </div>
        ...
        {(seed.images && seed.images.length > 1) && (
          <div className="p-4 bg-white border-b border-stone-200 flex gap-2 overflow-x-auto scrollbar-hide">
            {seed.images.map((img: string, idx: number) => (
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white p-3.5 rounded-xl shadow-sm border border-stone-100"><div className="text-stone-400 text-[10px] font-bold uppercase tracking-wider mb-1">Maturity</div><div className="text-stone-800 font-bold">{seed.days_to_maturity} Days</div></div>
            <div className="bg-white p-3.5 rounded-xl shadow-sm border border-stone-100"><div className="text-stone-400 text-[10px] font-bold uppercase tracking-wider mb-1">Sunlight</div><div className="text-stone-800 font-bold truncate">{seed.sunlight || '--'}</div></div>
            <div className="bg-white p-3.5 rounded-xl shadow-sm border border-stone-100"><div className="text-stone-400 text-[10px] font-bold uppercase tracking-wider mb-1">Life Cycle</div><div className="text-stone-800 font-bold truncate">{seed.lifecycle || '--'}</div></div>
            <div className="bg-white p-3.5 rounded-xl shadow-sm border border-stone-100"><div className="text-stone-400 text-[10px] font-bold uppercase tracking-wider mb-1">Vendor</div><div className="text-stone-800 font-bold truncate">{seed.vendor || '--'}</div></div>
          </div>
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm border border-emerald-100 relative overflow-hidden">
              ...
              <div className="space-y-3">
                {seedHistory.map((tray: SeedlingTray) => {
                   const seedData = tray.contents.find(c => c.seed_id === seed.id);
          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-stone-100">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between border-b border-stone-50 pb-2"><span className="text-stone-500">Seed Depth</span><span className="font-medium text-stone-800">{seed.seed_depth || '--'}</span></div>
              <div className="flex justify-between border-b border-stone-50 pb-2"><span className="text-stone-500">Plant Spacing</span><span className="font-medium text-stone-800">{seed.plant_spacing || '--'}</span></div>
              <div className="flex justify-between border-b border-stone-50 pb-2"><span className="text-stone-500">Row Spacing</span><span className="font-medium text-stone-800">{seed.row_spacing || '--'}</span></div>
              <div className="flex justify-between border-b border-stone-50 pb-2"><span className="text-stone-500">Germination Time</span><span className="font-medium text-stone-800">{seed.germination_days || '--'}</span></div>
              {(seed.light_required || seed.cold_stratification) && (
                <div className="flex flex-wrap gap-2 pt-2">
                  {seed.light_required && <span className="bg-amber-100 text-amber-800 text-xs font-bold px-2 py-1 rounded">Needs Light to Germinate</span>}
                  {seed.cold_stratification && <span className="bg-blue-100 text-blue-800 text-xs font-bold px-2 py-1 rounded">Cold Stratification ({seed.stratification_days} days)</span>}
                </div>
              )}
            </div>
          </div>
          ...
          {seed.companion_plants && seed.companion_plants.length > 0 && (
            <div className="bg-emerald-50 p-4 rounded-xl shadow-sm border border-emerald-100">
              ...
              <div className="flex flex-wrap gap-1.5">
                {seed.companion_plants.map((comp: string) => <span key={comp} className="bg-white border border-emerald-200 text-emerald-700 text-xs font-medium px-2.5 py-1 rounded-full">{comp}</span>)}
              </div>
            </div>
          )}
          ...
          {seed.notes && (
            <div className="bg-white p-5 rounded-xl shadow-sm border border-stone-100">
              ...
              <p className="text-stone-600 text-sm leading-relaxed whitespace-pre-wrap">{seed.notes}</p>
            </div>
          )}
        </div>
      </div>
