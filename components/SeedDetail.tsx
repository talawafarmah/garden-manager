...
export default function SeedDetail({ seed, trays, categories, navigateTo, handleGoBack }: any) {
  ...
  useEffect(() => {
    const loadUrls = async () => {
      ...
      for (const img of fullSeed.images) {
        // If it's NOT a base64 string and NOT a standard http link, resolve it as a bucket path
  ...
  const handleDuplicateSeed = async () => {
    ...
    try {
      ...
      // Navigate straight to the edit screen with the duplicated metadata
  ...
  /**
   * DYNAMIC BACK LOGIC:
   * Uses the 'returnTo' metadata passed during navigation from components like TrayDetail.
   * If arriving from a tray, it snaps back to that specific tray using the returnPayload.
   * If arriving from the vault (default), it returns to the vault list.
   */
  const onBack = () => {
    if (seed.returnTo) {
  ...
  // Handle main image display logic
  ...
  // Filter trays to show where this specific variety has been used
    ...
    <main className="min-h-screen bg-stone-50 text-stone-900 pb-20">
      ...
      <header className="bg-emerald-700 text-white p-4 shadow-md sticky top-0 z-10 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
           ...
           <button onClick={() => navigateTo('seed_edit', fullSeed)} className="bg-emerald-800 hover:bg-emerald-600 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors border border-emerald-600 shadow-sm flex items-center gap-1.5">
        ...
        {fullSeed.images && fullSeed.images.length > 1 && (
          <div className="p-4 bg-white border-b border-stone-200 flex gap-2 overflow-x-auto scrollbar-hide">
            {fullSeed.images.map((img: string, idx: number) => {
              ...
              return (
                <div key={idx} onClick={() => setViewingImageIndex(idx)} className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-all ${idx === viewingImageIndex ? 'border-emerald-500 opacity-100 shadow-md' : 'border-transparent opacity-60 hover:opacity-100'}`}>
        ...
        <div className="p-4 space-y-4">
                   ...
                   {seedHistory.map((t: SeedlingTray) => {
                     ...
                     return (
                       <button 
                         ...
                         className="w-full bg-stone-50 p-3 rounded-xl text-xs flex justify-between items-center border border-stone-100 hover:bg-stone-100 active:scale-95 transition-all"
                       >
                         <div className="flex flex-col items-start">
                           <span className="font-bold text-stone-700 truncate max-w-[150px]">{t.name}</span>
           ...
           <section className="bg-white p-5 rounded-2xl shadow-sm border border-stone-200">
             ...
             <div className="space-y-2.5 text-sm">
                ...
                {fullSeed.light_required && <div className="text-[10px] font-black text-amber-600 uppercase flex items-center gap-1.5 mt-1 pt-1 border-t border-stone-50"><svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0zm-.464 4.95l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm2.12-10.607a1 1 0 010 1.414l-.706.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM17 11a1 1 0 100-2h-1a1 1 0 100 2h1zm-7 4a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zM5.05 6.464A1 1 0 106.465 5.05l-.708-.707a1 1 0 00-1.414 1.414l.707.707zm1.414 8.486l-.707.707a1 1 0 01-1.414-1.414l.707-.707a1 1 0 011.414 1.414zM4 11a1 1 0 100-2H3a1 1 0 000 2h1z" /></svg> Light Required to Sprout</div>}
             </div>
           </section>
